var Global = function() {
	...
	return {
		...
		bindBtnSendSmsCode: function() {
			$("body").on("click", ".btn-send-sms-code", function() {
				var timer;
				var $sendCode = $(this);
				if ($sendCode.attr("disabled")) {
					return
				}
				var $mobile = $sendCode.closest("form").find("input[name='" + $sendCode.attr("data-mobile-el") + "']");
				if ($mobile.valid()) {
					var captchaImgId = "ModalView_captchaImg";
					var captchaImgTemplete = '<div class="model-background" style="z-index: 1050; display: block;" tabindex="-1"></div><div class="text-center" style="position: fixed; top: 30%; right: 0; left: 0; z-index: 1051;"><div class="modal-dialog"><div class="modal-content" style="color: #000000; background: #ffffff!important"><div class="modal-header"><h4 class="modal-title">验证码</h4></div><div class="modal-body"><div class="row"><div class="col-md-12"><div class="input-group"><input class="form-control captcha-text" type="text" autocomplete="off" placeholder="请输入验证码" name="captcha" required="true" data-msg-required="请填写验证码" /><span class="input-group-btn" style="cursor: pointer;"> <img alt="验证码" class="captcha-img" src="' + WEB_ROOT + '/static/img/captcha_placeholder.jpg" title="看不清？点击刷新" /></span></div></div></div></div><div class="modal-footer"><button type="button" class="btn btn-primary btn-block">确定</button></div></div></div></div>';
					$(document.body).append("<div id='" + captchaImgId + "'>" + captchaImgTemplete + "</div>");
					var $modalView = $("#" + captchaImgId).find("div");
					var $captchaImage = $modalView.find(".captcha-img");
					$captchaImage.click();
					var captcha = "";
					$modalView.find("button").unbind().bind("click", function(e) {
						var $captcha = $modalView.find('input[name="captcha"]');
						captcha = $captcha.val();
						console.log(captcha);
						if (captcha == "") {
							alert("验证码不能为空");
							...
							$sendCode.html("正在发送...");
							$sendCode.closest(".form-group").find(":text:first").focus().select();
							var open = $sendCode.attr("data-open");
							$.ajax({
								url: WEB_ROOT + "/" + (open && open != "false" ? "send" : "user") + "-sms-code/" + $mobile.val() + "?code=" + captcha,
								dataType: "json",
								method: "get",
								success: function(response) {
									if (response.type == "success") {
										$("#" + captchaImgId).remove();
										$sendCode.html("<i>已发送<span class='small'>(<span class='count'> 60 </span>秒后可重发)</span></i>");
										timer = setInterval(function() {
											var count = $sendCode.find(".count").html();
											if (count <= 0) {
												...
												$sendCode.html("<i>已发送<span class='small'>(<span class='count'> " + count + " </span>秒后可重发)</span></i>")
											}
										}, 1000)
									} else {
		...
		bindTableInfiniteScroll: function() {
			var infiniteScrolHandler = function() {
				...
				if ($tables.size() > 0 && $(this).scrollTop() + $(window).height() + 20 >= $(document).height()) {
		...
		bindExtBootstrapTab: function() {
			...
			$(document).on("click.tab.data-api", '[data-toggle="tab"], [data-toggle="pill"]', function(e) {
				...
				var href = $a.attr("data-url") ? $a.attr("data-url") : $a.attr("href");
				href = $.trim(href);
				if (!Util.startWith(href, "#")) {
					$a.attr("data-url", href);
					var contentId = "tab_content_" + Util.hashCode(href);
					$a.attr("href", "#" + contentId);
					var $navParent = $a.closest("ul.nav").parent();
					var $contents = $navParent.find(" > div.tab-content");
					if ($contents.length == 0) {
						$contents = $('<div class="tab-content">').appendTo($navParent)
					}
					...
					if ($content.is(":empty")) {
						$content.ajaxGetUrl(href, function() {
							...
							$content.find(".nav > li.active > a").click()
						})
					}
				}
				$(this).tab("show");
				var $navTabs = $(this).closest(".nav-tabs");
				var idx = $navTabs.find("li:not(.tools)").index($(this).parent("li"));
		...
		bindCaptchaCode: function() {
			jQuery("body").on("click", ".captcha-img", function(e) {
				$(".captcha-img").each(function() {
					$(this).attr("src", WEB_ROOT + "/static/img/captcha_placeholder.jpg")
				});
			...
			jQuery("body").on("focus", ".captcha-text", function(e) {
				...
				if ($captchaImg.attr("src") == WEB_ROOT + "/static/img/captcha_placeholder.jpg") {
					$captchaImg.click()
				}
			})
		},
		bindBtnPostUrl: function() {
			jQuery("body").on("click", ".btn-post-url", function(e) {
				...
				if (prompt) {
					confirmMsg = false;
					var promptDone = $btn.attr("data-prompt-done");
						...
						bootbox.prompt(prompt, function(result) {
							if (result === null) {} else {
								if ($.trim(result) == "") {
									...
									$btn.attr("data-prompt-done", true);
									$btn.attr("data-url", Util.AddOrReplaceUrlParameter(url, "prompt", result));
									$btn.click()
								}
							}
							return
				...
				$btn.ajaxPostURL({
					url: url,
					success: function(response) {
						var success = $btn.attr("data-post-success");
						if (success) {
							eval(success);
							return
						}
						var reload = $btn.attr("data-post-reload");
						if (reload) {
							if (reload == "page") {
		...
		triggerSingleFileUpload: function($el) {
			...
			if (dataZoomoutTo > 0) {
				$('<input type="hidden" name="data-zoomout-to" value="' + dataZoomoutTo + '"/>').insertAfter($("input[name='fileUpload']", $singleFileUploadForm))
			}
		...
		init: function() {
			...
			if (navId && navId != "none") {
				$(".header a[data-nav-id='" + navId + "']").parent("li").addClass("active")
			}
			toastr.options = {
				tapToDismiss: false,
				closeButton: true,
			...
			if ($menuContainer.size() > 0) {
				$.address.change(function(event) {
					...
					if (hash == "/") {
						...
						var $a = $menuContainer.find('li.menu > a[rel="address:' + event.value + '"]').first();
						if ($a.size() == 0) {
							$a = $menuContainer.find('li.menu > a[rel^="address:' + event.value + '"]').first()
						}
					}
					if ($a.size() == 0) {
						$(".page-menu-content").ajaxGetUrl(WEB_ROOT + event.value)
					} else {
						var href = WEB_ROOT + $.trim($a.attr("rel").replace("address:", ""));
						...
						if (!$li.parent().hasClass("page-sidebar-menu")) {
							...
							while ($ul.size() > 0) {
								...
								$pli.addClass("open");
								$pli.find(" > a > i.fa").removeClass("fa-caret-right");
								$pli.find(" > a > i.fa").addClass("fa-caret-down");
			...
			$("body").on("click", ".paging_bootstrap a", function() {
				var $a = $(this);
				var $pager = $a.closest(".paging_bootstrap");
				if ($pager.attr("target") == undefined || $pager.attr("target") == "ajax-get-container") {
					var $container = $a.closest(".ajax-get-container");
					var url = $container.attr("data-url");
					$container.ajaxGetUrl(Util.AddOrReplaceUrlParameter(url, "page", $a.attr("page")))
				} else {
					var $target = $($pager.attr("target"));
					if ($target.is("form")) {
						var $pageNo = $target.find("input[name='page']");
						if ($pageNo.size() == 0) {
							$pageNo = $("<input type='hidden' name='page'/>").appendTo($target)
						}
			...
			jQuery("body").on("click", '[data-toggle="buttons"] > label.btn', function(e) {
				var $btn = $(this);
				if ($btn.is(".clear")) {
					$(this).parent().find("> label.btn:not(.clear)").removeClass("active");
					$(this).parent().find("> label.btn:not(.clear) > input[type='checkbox']").attr("checked", false)
				} else {
					$(this).parent().find("> label.btn.clear").removeClass("active")
				}
			});
			jQuery("body").on("click", '[data-toggle="buttons"] > label.btn', function(e) {
				$(this).parent().find("> label.clear").removeClass("active")
			});
			...
			jQuery("body").on("click", ".btn-file-upload", function(e) {
				var $btn = $(this);
				var id = $btn.attr("id");
				if (id == undefined) {
				...
				e.preventDefault();
				$btn.popupDialog({
					url: WEB_ROOT + "/w/image/upload?el=" + id + "&_" + new Date().getTime(),
					title: "图片上传",
					size: 600
				})
			});
			$("input[name='fileUpload']", $singleFileUploadForm).change(function() {
				var $b = $singleFileUploadTrigger.parent();
				App.blockUI($b);
				$singleFileUploadForm.ajaxSubmit({
					dataType: "json",
					method: "post",
					success: function(response) {
						if (response.type == "success") {
							var src = response.data;
							$singleFileUploadTrigger.val(src);
							var $form = $singleFileUploadTrigger.closest("form");
							$form.data("validator").element($singleFileUploadTrigger);
							var name = $singleFileUploadTrigger.attr("name");
							var $img = $singleFileUploadTrigger.closest(".form-group").find("img.image-display[data-to='" + name + "']");
							if ($img.size() > 0) {
							...
							var dataGridReload = $singleFileUploadTrigger.attr("data-grid-reload");
							if (dataGridReload) {
					...
					error: function(xhr, e, status) {
						Global.notify("error", "文件上传处理异常，请联系管理员");
						App.unblockUI($b)
					}
				});
				return false
			...
			$("body > .page-container-outer").css("min-height", $(window).height() - $("body > .header").outerHeight() - $("body > .footer").outerHeight() - 55);
			$("body > .footer").show();
			$(document).on("mousedown", function(e) {
				var $preview = $("#image-preview");
				if ($preview.size() > 0) {
					if (!($preview.is(e.target) || $preview.find(e.target).length)) {
						$preview.hide()
					}
				}
				var $btnProfileParam = $("#btn-profile-param");
				if ($btnProfileParam.size() > 0) {
					if (!($btnProfileParam.is(e.target) || $btnProfileParam.find(e.target).length)) {
		...
		showModel: function(message, opt) {
			...
			var modaltemplate = '<div class="model-background" style="z-index: 1050;" tabindex="-1"></div><div class="text-center" style="position: fixed; top: 35%; right: 0; left: 0; z-index: 1051;"><div class="modal-dialog"><div class="modal-content ui-draggable"><div class="modal-body"><h1 class="col-md-12" style="color:white;"><img src="' + WEB_ROOT + "/static/w/app/images/" + options.icon + '"> ' + options.title + '</h1><div class="col-md-12 margin-top-20">MESSAGE</div><div class="col-md-12 margin-top-20 margin-bottom-20"><a href="javascript:;" close="#ModalView_1" class="btn btn-action confirm-btn ok-ModalView_1" style="min-width:120px;">' + options.button + "</a></div></div></div></div></div>";
			var id = "ModalView_" + (vuuid++);
			message = (message.indexOf("modal-body") > 0) ? message : '	<div class="modal-body" style="color:white;">' + message + "</div>";
			message = message.replace(/data-dismiss="modal"/gi, "close='#" + id + "'");
			$(document.body).append("<div id='" + id + "'>" + modaltemplate.replace(/modaltemplate/gi, id).replace(/z-index: 105/gi, "z-index: " + (99 + vuuid)).replace("MESSAGE", message) + "</div>");
