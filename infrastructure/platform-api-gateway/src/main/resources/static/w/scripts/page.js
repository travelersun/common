	...
	return {
		...
		initDropdownSelect: function($container) {
			$('input[data-toggle="dropdown-select"]', $container).each(function() {
				...
				$elems.wrapAll('<div class="input-group" style="width:100%"><div class="input-icon right"></div></div>');
				...
				$('select[data-toggle="double-multi-select"]', $container).each(function() {
					...
					$select.multiSelect({
						...
						afterInit: function(ms) {
							...
							if (height) {
								that.$container.find(".ms-list").css({
									height: height
								})
							}
						},
		...
		initSelect2: function($container) {
			if (jQuery().select2) {
				$("select", $container).each(function() {
					...
					if ($select2.is('[data-toggle="double-multi-select"]')) {
						return this
					}
					...
					var cascadeParent = $select2.attr("data-cascade-parent");
					if (cascadeParent) {
						var $cascadeParent = $select2.closest("form").find("select[name='" + cascadeParent + "']");
						$select2.data("cachedOptions", $select2.children("option").clone());
						var cascadeParentVal = $cascadeParent.val();
						if (cascadeParentVal && cascadeParentVal != "") {
							$select2.children("option").each(function(i, option) {
								var $option = $(this);
								var parentValue = $option.attr("data-cascade-parent");
								if (parentValue && parentValue != cascadeParentVal) {
									$option.remove()
								}
							})
						} else {
					...
					if ($select2.attr("data-select2-type") == "combobox") {
						...
						$select2.on("change", function(e) {
							...
							var cascadeName = $select2.attr("data-cascade-name");
							if (cascadeName) {
								var $cascadeTarget = $select2.closest("form").find("select[name='" + cascadeName + "']");
								$cascadeTarget.empty();
								if (val && val != "") {
									var parent = $cascadeTarget.attr("data-cascade-parent");
									if (parent) {
										$cascadeTarget.data("cachedOptions").each(function() {
											var $option = $(this);
											var parentValue = $option.attr("data-cascade-parent");
											if (parentValue == undefined || parentValue == val) {
												$cascadeTarget.append($option);
												$cascadeTarget.select2(options)
											}
										})
									} else {
										var cachedData = $cascadeTarget.data(val);
										if (cachedData) {
											$cascadeTarget.append(cachedData);
											$cascadeTarget.select2(options)
										} else {
											var url = $select2.attr("data-cascade-url");
											var pvalue = val;
											var pname = $select2.attr("name");
											var property = $select2.attr("data-cascade-property");
											if (property) {
												var $selectoption = $select2.find("option[value='" + val + "']");
												pname = property;
												pvalue = $selectoption.attr("data-" + property)
											}
											url = Util.AddOrReplaceUrlParameter(url, pname, pvalue);
											$cascadeTarget.ajaxJsonUrl(url, function(response) {
												var arr = [];
												arr.push("<option value=''></option>");
												for (var key in response) {
													arr.push("<option value='" + key + "'>" + response[key] + "</option>")
												}
												cachedData = arr.join("");
												$cascadeTarget.append(cachedData);
												$cascadeTarget.select2(options);
												$cascadeTarget.data(val, cachedData)
											})
										}
									}
								}
				...
				$('input[data-select2-type="remote"]', $container).each(function() {
					...
					var options = {
						...
						formatSelection: function(item) {
							var val = item.id;
							var valTmpl = $select2.attr("data-val-tmpl");
							if (valTmpl) {
								val = $.tmpl(valTmpl, item)[0].data
							}
							$select2.val(val);
							var display = item.display;
							var displayTmpl = $select2.attr("data-display-tmpl");
							if (displayTmpl) {
								display = $.tmpl(displayTmpl, item)
							}
							return display
						}
					};
		...
		initDatePicker: function($container) {
			$('input[data-picker="date"]', $container).each(function() {
				...
				$datapicker.wrap('<div class="input-group"><div class="input-icon right"></div></div>');
				...
				var todayBtn = true;
				var dataTodayBtn = $datapicker.attr("data-today-btn");
		...
		initDateRangePicker: function($container) {
			$('input[data-picker="date-range"]', $container).each(function() {
				...
				if (val == "") {
					var initVal = $datapicker.attr("data-date-init");
				...
				$datapicker.wrap('<div class="input-group" style="width:100%"><div class="input-icon right"></div><div>');
				$datapicker.before('<i class="fa fa-calendar"></i>');
		...
		initUploadImage: function($container) {
			$('input[data-upload="image"]', $container).each(function() {
				var $el = $(this);
				...
				if ($el.attr("id") == undefined) {
					$el.attr("id", "image_id_" + new Date().getTime())
				}
				var options = {
					"vertical-align": "top",
					"max-width": "200px",
				...
				if ($el.attr("data-max-width")) {
					options["max-width"] = $el.attr("data-max-width");
					if (!Util.endWith(options["max-width"], "px") && !Util.endWith(options["max-width"], "%")) {
						options["max-width"] = options["max-width"] + "px"
					}
				}
				if ($el.attr("data-max-height")) {
					options["max-height"] = $el.attr("data-max-height");
					if (!Util.endWith(options["max-height"], "px") && !Util.endWith(options["max-height"], "%")) {
						options["max-height"] = options["max-height"] + "px"
					}
				}
				if (Global.isIELowerVersion()) {
					var $trigger = $('<a  href="javascript:;"></a>').insertAfter($el);
					var src = $el.val();
					if (src != "") {
						if (READ_FILE_URL_PREFIX) {
							src = READ_FILE_URL_PREFIX + src
						}
						$img = $('<img class="image-display" data-to="' + $el.attr("name") + '"  src="' + src + '">').appendTo($trigger);
						$img.css(options);
						$img.css({
							"margin-left": "90px"
						})
					...
					$file.attr("style", "position:absolute;left:0;top:0;width:85px;height:34px;z-index:999;");
					...
					if (dataZoomoutTo > 0) {
						url += "?data-zoomout-to=" + dataZoomoutTo
					}
						...
						$form.ajaxSubmit({
							...
							success: function(response) {
								...
								if (response.type == "success") {
									...
									$form.data("validator").element($el);
									var $img = $el.closest(".form-group").find("img.image-display");
									if ($img.size() == 0) {
										$img = $('<img class="image-display" >').appendTo($trigger);
										$img.css(options);
										$img.css({
											"margin-left": "90px"
										})
									...
									$img.attr("src", src);
									var dataGridReload = $el.attr("data-grid-reload");
									if (dataGridReload) {
					...
					var src = $el.val();
					if (src == "") {
						src = WEB_ROOT + "/static/w/app/images/upload.jpg"
					} else {
						if (READ_FILE_URL_PREFIX) {
							src = READ_FILE_URL_PREFIX + src
						}
					}
					var $trigger = $('<a  href="javascript:;"><img class="image-display" data-to="' + $el.attr("name") + '" src="' + src + '"></a>').insertAfter($el);
					$trigger.children("img").css(options);
		...
		initNavbar: function($container) {
			$(".navigator", $container).each(function() {
				var $el = $(this);
				var navs = {
					"申请": ["申请代理", "验证", "签署兼职协议", "填写信息", "提交成功"],
					"审核": ["查询审核进度", "审核结果", "登录蜡笔分期", "签约", "下载推广APP", "关注蜡笔分期"]
				};
				var htm = '<span class="active">首页</span>';
				var steps = ("审核" !== $el.data("path")) ? navs["申请"] : navs["审核"];
				var current = $el.data("location") || steps[0];
				var css = ' class="active"';
				for (var i in steps) {
					htm += '<span class="divider">&gt;</span><span' + css + ">" + steps[i] + "</span>";
					if (current == steps[i]) {
						css = ""
					}
					if (current == "查询审核进度" || current == "审核结果") {
						break
					}
				}
				$el.html(htm)
		...
		initAjaxBeforeShow: function($container) {
			...
			Page.initUploadImage($container);
			Page.initImagePreivew($container);
			Page.initSlimscroll($container);
			Page.initNavbar($container);
