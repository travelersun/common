var AdminGlobal = function() {
	...
	return {
			...
			if (b.size() > 0) {
				$.ajax({
					...
					success: function(d) {
						var e = d.data;
						if (e > 0) {
							var f = "<a href='javascripts:;' rel='address:/admin/profile/notify-message?readed=no|未读公告信息列表'>您有 " + e + " 条未读公告信息</a>";
			...
			if (c.size() > 0) {
				$.ajax({
					...
					success: function(d) {
						var e = d.data;
						if (e > 0) {
							var f = "<a href='javascripts:;' rel='address:/admin/profile/user-message?readed=no|未读个人消息列表'>您有 " + e + " 条未读个人消息</a>";
		...
		init: function() {
			...
			if (e.size() > 0) {
				var f = $(window).height() - e.offset().top - $("div.footer").outerHeight() - 8;
				e.css({
					"min-height": f
				})
			}
			...
			var c = $(window).height() - $("body > .header").height() - $("body > .footer").height() - 15;
			...
			$("div#portlet-layout > .portlet-title-layout > .tools > .reload").click(function(m) {
				var i = $("div#portlet-layout").find(" > .portlet-body > .portlet-tabs");
			...
			if (b.size() > 0) {
				var h = function(m, o) {
					...
					var i = '<li class="menu ' + n + " " + l + '" data-path="' + m.path + '">';
					i += '<a href="javascript:;" ' + (k ? ' data-py="' + k + '"' : "") + (m.url ? ' rel="address:' + m.url + "|" + m.path + '"' : "") + ">";
					if (m.style) {
				...
				b.ajaxJsonUrl(WEB_ROOT + "/admin/menus", function(k) {
					...
					$.each(k, function(o, p) {
						...
						if (q.length == 1) {
							...
							if (l.size() == 0) {
								l = $('<ul class="sub-menu" style="display: ' + (r.is(".open") ? "block" : "none") + ';">').appendTo(r)
							}
							l.append(h(p, q))
						}
					})
				});
				$.address.change(function(l) {
					if (l.value == "/dashboard") {
						...
						if (l.value == "/") {
							...
							if (l.value == "/lock") {
								$.backstretch(["static/img/bg/1.jpg", "static/img/bg/2.jpg", "static/img/bg/3.jpg", "static/img/bg/4.jpg"], {
									fade: 1000,
									duration: 8000
								});
								...
								var m = $('.page-sidebar li.menu > a[rel^="address:' + l.value + '"]');
								if (m.size() > 0) {
									AdminGlobal.addOrActivePanel(m.attr("rel").replace("address:", ""));
									...
									AdminGlobal.addOrActivePanel(l.value)
								}
							}
						}
					}
			...
			jQuery("body").on("click", "#layout-nav >  li > .btn-close-active", function(l) {
				var k = $("#layout-nav");
				var j = k.next(".tab-content").find(".panel-content:visible").attr("data-url");
				var i = k.find(" > .btn-group > ul.dropdown-menu");
				i.find("a[href='" + j + "']").find(".badge").click()
			});
			jQuery("body").on("click", "ul.nav > li.tools > .reload", function(m) {
				m.preventDefault();
				var i = $(this).closest(".nav");
				var l = i.find("li.active > a");
				var k = i.closest(".tabbable").find(l.attr("href"));
				if (l.attr("data-url")) {
					var j = l.attr("data-url");
					k.ajaxGetUrl(j, function() {
						k.find(".tabbable:first > .nav > li.active > a").click()
					})
				} else {
					if (jQuery().jqGrid) {
						k.find("table.ui-jqgrid-btable").each(function() {
							var n = $(this);
			...
			$(".page-sidebar, .header").on("click", ".sidebar-toggler", function(i) {
				Grid.refreshWidth()
			});
			if ($.fn.fileupload) {
				...
				var g = $("#fileupload");
				var d = null;
				jQuery("body").on("click", "a.btn-fileinput-trigger", function(j) {
					d = $(this);
					var i = d.attr("data-category");
					if (i) {
						g.find("input[name='attachmentName']").val("_attachment_" + i)
					}
					g.find("tbody.files").empty()
				});
				jQuery("#fileupload-dialog").on("click", ".modal-footer .btn-add", function(k) {
					var j = d.parent().find("table.table-filelist");
					if (j.size() == 0) {
						j = $('<table role="presentation" class="table table-striped table-filelist clearfix"><tbody class="files"></tbody></table>').insertAfter(d)
					}
			...
			jQuery("body").on("click", 'a[data-toggle="panel"],button[data-toggle="panel"]', function(j) {
				j.preventDefault();
				var i = $(this);
				AdminGlobal.addOrActivePanel(i)
			});
			jQuery("body").on("click", 'a[data-upload="btn-single-file"],button[data-upload="btn-single-file"]', function(j) {
				j.preventDefault();
				var i = $(this);
				Global.triggerSingleFileUpload(i)
			});
			jQuery("body").on("click", "tbody.select-table-checkbox", function(j) {
		...
		findUserProfileParams: function(b) {
			if (a == undefined || a == null) {
				a = $("body").cacheData(WEB_ROOT + "/admin/sys/user-profile-data/params.json")
			}
			return a[b]
		},
